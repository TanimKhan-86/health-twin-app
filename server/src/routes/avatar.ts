...
/** Derive server base URL from the incoming request (e.g. http://localhost:4000). */
...
router.post('/setup', requireAuth, upload.single('photo'), async (req: AuthRequest, res) => {
    try {
        ...
        if (AVATAR_MODE === 'prebuilt') {
            ...
            const setupBaseUrl = getBaseUrl(req);
            const rawSetupImageUrl = isLoadableUrl(avatar?.avatarImageUrl) ? avatar.avatarImageUrl : null;
            // Accept all animations with loadable videoUrls (including media://)
            const generatedStates = Array.from(
                new Set(
                    animations
                        .filter(anim => isLoadableUrl(anim.videoUrl))
            ...
            sendSuccess(res, {
                ...
                imageUrl: resolveMediaUrl(rawSetupImageUrl, setupBaseUrl),
                profileImage: resolveMediaUrl(user.profileImage, setupBaseUrl),
                generatedStates,
...
router.get('/state', requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
        ...
        // Accept all loadable URLs including media:// GridFS references
        const rawAvatarImageUrl = isLoadableUrl(avatar?.avatarImageUrl) ? avatar.avatarImageUrl : null;
        // Fingerprint is only meaningful for data: or http(s) URLs, not media:// references
        const avatarFingerprint = rawAvatarImageUrl && !rawAvatarImageUrl.startsWith('media://')
            ? computeAvatarFingerprint(rawAvatarImageUrl)
            : null;

        // Accept all animations with loadable videoUrls (including media://)
        const usableAnimations = animations.filter(anim => isLoadableUrl(anim.videoUrl));

        // Build animation map, resolving media:// URIs to server URLs
        const animMap = usableAnimations.reduce((acc, anim) => {
            const resolved = resolveMediaUrl(anim.videoUrl, baseUrl);
            if (resolved) {
        ...
        const fallbackVideo =
            ...
            (usableAnimations[0]?.videoUrl ? resolveMediaUrl(usableAnimations[0].videoUrl, baseUrl) : null) ||
            null;
        const resolvedReasoning = requestedState
            ? `Client preview requested "${requestedState}" state`
            : reasoning;

        // Resolve avatar image URL for client consumption
        const resolvedImageUrl = resolveMediaUrl(rawAvatarImageUrl, baseUrl);

        if (!fallbackVideo && !resolvedImageUrl) {
...
router.get('/status', requireAuth, async (req: AuthRequest, res) => {
    try {
        ...
        const statusBaseUrl = getBaseUrl(req);
        const rawStatusAvatarUrl = isLoadableUrl(avatar?.avatarImageUrl) ? avatar.avatarImageUrl : null;
        const generatedStates = Array.from(
            new Set(
                animations
                    .filter(anim => isLoadableUrl(anim.videoUrl))
        ...
        sendSuccess(res, {
            hasAvatar: !!avatar,
            avatarUrl: resolveMediaUrl(rawStatusAvatarUrl, statusBaseUrl),
...
router.get('/library', requireAuth, async (req: AuthRequest, res) => {
    try {
        ...
        const libBaseUrl = getBaseUrl(req);
        const rawLibAvatarUrl = isLoadableUrl(avatar?.avatarImageUrl) ? avatar.avatarImageUrl : null;
        const libUsableAnimations = animations.filter(anim => isLoadableUrl(anim.videoUrl));

        const videoByState = libUsableAnimations.reduce((acc, anim) => {
            const resolved = resolveMediaUrl(anim.videoUrl, libBaseUrl);
            if (resolved) {
                acc[anim.stateType] = resolved;
            }
            return acc;
        ...
        sendSuccess(res, {
            mode: AVATAR_MODE,
            requiredStates: ACTIVE_AVATAR_STATES,
            imageUrl: resolveMediaUrl(rawLibAvatarUrl, libBaseUrl),
            videoByState,
