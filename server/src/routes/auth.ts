...
import { resolveMediaUrl } from '../services/avatarMediaService';
...
const registerSchema = z.object({
    ...
    profileImage: z.string().trim().min(1).optional().refine(
        (val) => !val || val.startsWith('data:') || val.startsWith('http://') || val.startsWith('https://'),
        { message: 'Profile image must be a data URI or HTTP URL' }
    ),
...
function toAuthUser(user: {
    ...
    return {
        ...
        profileImage: baseUrl
            ? resolveMediaUrl(user.profileImage, baseUrl)
...
router.post('/register', async (req, res: Response): Promise<void> => {
    ...
    try {
        ...
        const regBaseUrl = `${req.protocol}://${req.get('host') || 'localhost:4000'}`;
        const payload: AuthSessionDto = {
            token,
            user: toAuthUser(user, regBaseUrl),
        };
...
router.post('/login', async (req, res: Response): Promise<void> => {
    ...
    try {
        ...
        const loginBaseUrl = `${req.protocol}://${req.get('host') || 'localhost:4000'}`;
        const payload: AuthSessionDto = {
            token,
            user: toAuthUser(user, loginBaseUrl),
        };
...
router.get('/me', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const meBaseUrl = `${req.protocol}://${req.get('host') || 'localhost:4000'}`;
        sendSuccess(res, { user: toAuthUser(user, meBaseUrl) });
    } catch (error: unknown) {
        sendError(res, 500, getErrorMessage(error));
    }
});
