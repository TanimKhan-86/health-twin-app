...
/** Returns true if the URL is a loadable resource (data: URI, http(s) URL, or media:// GridFS ref). */
export function isLoadableUrl(url?: string | null): url is string {
    if (!url) return false;
    return url.startsWith('data:') || url.startsWith('http://') || url.startsWith('https://') || url.startsWith('media://');
}

const MEDIA_PROTOCOL = 'media://';

/**
 * Resolves a `media://<gridfs-id>` URI to a full server URL (`/api/media/<id>`).
 * Passes through data: and http(s) URIs unchanged. Returns null for invalid URIs.
 */
export function resolveMediaUrl(url: string | null | undefined, baseUrl: string): string | null {
    if (!url) return null;
    if (url.startsWith(MEDIA_PROTOCOL)) {
        const id = url.slice(MEDIA_PROTOCOL.length);
